...
def build_model(n_hidden, n_neurons_hidden, n_neurons_output, learning_rate) -> keras.models.Sequential:
...
for layers in n_hidden:                    #looking or the best parameters w.r.t the number of hidden layers
    for neurons_hidden in n_neurons:       #looking or the best parameters w.r.t the number of neurons
        for lr in l_rate:                  #looking or the best parameters w.r.t the learning rate
            ...
            model = build_model(layers, neurons_hidden, 10, lr)
            #To train the model
            ...
            for prediction, truth in zip(class_predicted, y_test):
                if prediction == truth:
                    correct_count += 1
            accuracy = correct_count / len(class_predicted)
